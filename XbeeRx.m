...
@synthesize startbyte,length,frametype,AT1,AT2,sourceAddr64,sourceAddr16,receiveoptions,receiveddata,checksum,nodeidentifier,parentnetworkaddress,devicetype,sourceAddr16HexString,sourceAddr64HexString,sourcePANid,sourcePANidHexString,frameID,ack,retries,startID,endID,msgType;
...
//    NSLog(@"the startbyte is %c",(char)[startbyte unsignedCharValue]);
    ...
    if ([frametype unsignedIntValue] == 136){
        ...
        if (([AT1 unsignedIntValue]== 73)&& ([AT2 unsignedIntValue]== 68)) { //if this is a ID command note that PAN ID are stored in sourcePANid field
            
            sourcePANid = [[NSMutableArray alloc] initWithCapacity:8];
            [sourcePANid addObject:[rxBuffer objectAtIndex:8]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:9]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:10]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:11]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:12]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:13]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:14]];
            [sourcePANid addObject:[rxBuffer objectAtIndex:15]];
        
            NSMutableString* PANid = [[NSMutableString alloc] initWithCapacity:8];
            for (int i =0; i<8; i++) {
                [PANid appendString:[NSString stringWithFormat:@"%.2x",[[sourcePANid objectAtIndex:i] unsignedIntValue]]];}
            sourcePANidHexString = PANid;   //update PANid field
            

             checksum = [rxBuffer objectAtIndex:16];//(NSString*)[rxstring characterAtIndex:16];
        }
    }
}
